...
using System.Linq;
using GithubTFSBridge.Providers;

// TODO: requirements
// Add link to github on TFS
// RDIssue and RDTask
// Ignore issues that were open within last 5 minutes so that
...
namespace GithubTFSBridge
{
    public class Bridge
    {
        ...
        private GitHubIssuesProvider GithubProvider { get; set; }
        private TfsIssuesProvider TfsIssuesProvider { get; set; }

        private bool SyncGithub { get; set; }
        private bool SyncTfs { get; set; }

        public Bridge (string githubUsername,
            ...
            bool syncTfs)
        {
            TfsIssuesProvider = new TfsIssuesProvider(tfsServerAddress, tfsProjectName, tfsAreaPath);
            GithubProvider = new GitHubIssuesProvider(githubUsername, githubPassword, githubOwner, githubRepository);
        ...
        public void Synchronize()
        {
            ...
            IList<WorkItem> workItems = TfsIssuesProvider.GetAll().ToList();
            IList<WorkItem> githubIssues = GithubProvider.GetAll().ToList();
            ...
            if (SyncTfs)
            {
                ...
                foreach (WorkItem githubIssue in githubIssues)
                {
                    // 
                }
            }
        ...
        {
            var tfs = new TfsTeamProjectCollection(new Uri(TfsServerAddress));
